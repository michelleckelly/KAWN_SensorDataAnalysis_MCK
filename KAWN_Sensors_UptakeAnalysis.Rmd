...
```{r Scale by aerial extent of metabolism modeling}
...
K600.eric <- readRDS("./Outputs/OxygenTurnover_Eric.RData")
K600.steve <- readRDS("./Outputs/OxygenTurnover_Steve.RData")
K600.desoto <- readRDS("./Outputs/OxygenTurnover_Desoto.RData")
# Change case of date column
K600.eric <- K600.eric %>% rename(Date = date)
K600.steve <- K600.steve %>% rename(Date = date)
K600.desoto <- K600.desoto %>% rename(Date = date)

# Merge oxygen turnover data with uptake data
eric_uptake_Eqn1 <- full_join(K600.eric, eric_uptake_Eqn1)
steve_uptake_Eqn1 <- full_join(K600.steve, steve_uptake_Eqn1)
...
# Supply = (Q * C) / (w * L)
#        = ([m3 / s] * [mg-N / L]) / ([m] * [m])
#        = [mg-N / (m2 sec)] * 86,400 [sec / day] * 1000 [L/m3] *1/1000 [g/mg]
#        = [g-N / m2 day]

# C     concentration of water column nitrate
#       needs to be pre-dawn maximum water column nitrate, before any uptake 
#       occurs

#Uptake_Eqn1$Supply_gNm2day.eric <- 
#  (Uptake_Eqn1$Mean.Discharge_m3s.eric * Uptake_Eqn1$Max.PreDawnN_mgL.eric) /
#  (100*w.Eric) * 86400
#Uptake_Eqn1$Supply_gNm2day.steve <- 
#  (Uptake_Eqn1$Mean.Discharge_m3s.steve * Uptake_Eqn1$Max.PreDawnN_mgL.steve) / 
#  (100*w.Steve) * 86400
#Uptake_Eqn1$Supply_gNm2day.desoto <- 
#  (Uptake_Eqn1$Mean.Discharge_m3s.desoto * Uptake_Eqn1$Max.PreDawnN_mgL.desoto) / 
#  (100*w.Desoto) * 86400

# Nix supply and percent change values whenever uptake values were bad
#Uptake_Eqn1$Supply_gNm2day.eric[is.na(Uptake_Eqn1$UaNO3_gNm2day.eric)] <- NA
#Uptake_Eqn1$Supply_gNm2day.steve[is.na(Uptake_Eqn1$UaNO3_gNm2day.steve)] <- NA
#Uptake_Eqn1$Supply_gNm2day.desoto[is.na(Uptake_Eqn1$UaNO3_gNm2day.desoto)] <- NA

#Uptake_Eqn1$NitrateChange_Perc.eric[is.na(Uptake_Eqn1$UaNO3_gNm2day.eric)] <- NA
#Uptake_Eqn1$NitrateChange_Perc.steve[is.na(Uptake_Eqn1$UaNO3_gNm2day.steve)] <- NA
#Uptake_Eqn1$NitrateChange_Perc.desoto[is.na(Uptake_Eqn1$UaNO3_gNm2day.desoto)] <- NA



```

```{r Depreciated: scale by set length (thesis)}
...
w.Desoto <- 138.07
  ...
  # These days will be termed "non-detectable", are likely due to changes in Q
  eric_uptake_Eqn1$UaNO3_gNm2day <- eric_uptake_Eqn1$SumDiff_mgNL *
    eric_uptake_Eqn1$Mean.Discharge_m3s / (w.Eric * 100) * 86400
  eric_uptake_Eqn1$UaNO3_gNm2day[eric_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
  eric_uptake_Eqn1$NitrateSupply_gNm2day <- eric_uptake_Eqn1$Mean.Nitrate_mgL*
    eric_uptake_Eqn1$Mean.Discharge_m3s * 86400 / (w.Eric * 100)
  
  # Steve
  steve_uptake_Eqn1$UaNO3_gNm2day <- steve_uptake_Eqn1$SumDiff_mgNL *
    steve_uptake_Eqn1$Mean.Discharge_m3s / (w.Steve * 100) * 86400
  steve_uptake_Eqn1$UaNO3_gNm2day[steve_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
  steve_uptake_Eqn1$NitrateSupply_gNm2day <- steve_uptake_Eqn1$Mean.Nitrate_mgL*
    steve_uptake_Eqn1$Mean.Discharge_m3s * 86400 / (w.Steve * 100)
  
  # Desoto
  # Set days where no uptake occured (negative values) to NA
  # These days will be termed "non-detectable", are likely due to changes in Q
  desoto_uptake_Eqn1$UaNO3_gNm2day <- desoto_uptake_Eqn1$SumDiff_mgNL *
    desoto_uptake_Eqn1$Mean.Discharge_m3s / (w.Desoto * 100) * 86400
  desoto_uptake_Eqn1$UaNO3_gNm2day[desoto_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
  desoto_uptake_Eqn1$NitrateSupply_gNm2day <- desoto_uptake_Eqn1$Mean.Nitrate_mgL *
    desoto_uptake_Eqn1$Mean.Discharge_m3s * 86400 / (w.Desoto * 100)
```

```{r Depreciated: depth scaling}
...
eric_uptake_Eqn1$A_m2 <- eric_uptake_Eqn1$calcdepth_m * w.Eric
steve_uptake_Eqn1$A_m2 <- steve_uptake_Eqn1$calcdepth_m * w.Steve
desoto_uptake_Eqn1$A_m2 <- desoto_uptake_Eqn1$calcdepth_m * w.Desoto
# Calculate uptake
...
Uptake_Eqn1$Supply_gNm2day.eric <- 
  (Uptake_Eqn1$Mean.Discharge_m3s.eric * Uptake_Eqn1$Max.PreDawnN_mgL.eric) /
  (Uptake_Eqn1$calcdepth_m.eric*w.Eric) * 86400
Uptake_Eqn1$Supply_gNm2day.steve <- 
  (Uptake_Eqn1$Mean.Discharge_m3s.steve * Uptake_Eqn1$Max.PreDawnN_mgL.steve) / 
  (Uptake_Eqn1$calcdepth_m.steve*w.Steve) *86400
Uptake_Eqn1$Supply_gNm2day.desoto <- 
  (Uptake_Eqn1$Mean.Discharge_m3s.desoto * Uptake_Eqn1$Max.PreDawnN_mgL.desoto) / 
  (Uptake_Eqn1$calcdepth_m.desoto*w.Desoto) *86400
```
...
Uptake_Eqn1 <- dplyr::full_join(Uptake_Eqn1, desoto_uptake_Eqn1, 
                                by = "Date")
# Rename columns
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(K600_daily_50pct.desoto = 
                                        K600_daily_50pct)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(depth_m.desoto = depth_m)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(Q_m3s.desoto = Q_m3s)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(A_m2.desoto = A_m2)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(v_ms.desoto = v_ms)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(v_mday.desoto = v_mday)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(O2TurnL_m.desoto = O2TurnL_m)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(O2TurnL_km.desoto = O2TurnL_km)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(Max.PreDawnN_mgL.desoto = 
                                        Max.PreDawnN_mgL)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(Min.Nitrate_mgL.desoto = Min.Nitrate_mgL)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(NitrateChange_Perc.desoto = 
                                        NitrateChange_Perc)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(Mean.Nitrate_mgL.desoto = 
                                        Mean.Nitrate_mgL)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(Mean.Discharge_m3s.desoto = 
                                        Mean.Discharge_m3s)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(SumDiff_mgNL.desoto = SumDiff_mgNL)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(A.desoto = A)
Uptake_Eqn1 <- Uptake_Eqn1 %>% rename(UaNO3_gNm2day.desoto = UaNO3_gNm2day)
...
write.csv(Uptake_Eqn1, 
          file = "./Outputs/NitrateUptakeResults_Eqn1.csv", 
          row.names = FALSE)
saveRDS(Uptake_Eqn1, file = "./Outputs/NitrateUptakeResults_Eqn1.rds")
