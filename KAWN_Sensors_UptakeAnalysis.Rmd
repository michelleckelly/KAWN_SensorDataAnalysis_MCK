...
if (UptakeEqn == 1) {
  ...
  HC_Eqn1 <- function(prepped_df){
    ...
    uptake_df <- 
      ...
      summarise(Mean.Discharge_m3s = mean(Mean.Discharge_m3s),
  ...
  # Calculate Ua with specific scaling for each site
  ...
  eric_uptake_Eqn1$SumDiff_mgNL[eric_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
  eric_uptake_Eqn1$UaNO3_gNm2day <- eric_uptake_Eqn1$SumDiff_mgNL *
    eric_uptake_Eqn1$Mean.Discharge_m3s / (210.9 * 100) * 86400
  
  # Steve
  steve_uptake_Eqn1$SumDiff_mgNL[steve_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
  ...
  # Set days where no uptake occured (negative values) to NA
  # These days will be termed "non-detectable", are likely due to changes in Q
  desoto_uptake_Eqn1$SumDiff_mgNL[desoto_uptake_Eqn1$SumDiff_mgNL < 0] <- NA
