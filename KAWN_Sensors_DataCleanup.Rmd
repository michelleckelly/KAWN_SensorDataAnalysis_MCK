...
masterFile <- function(sitename, USGS){
  ...
  if(file.exists(paste("KAWN_SensorData_", sitename, ".csv", sep = ""))){
    ...
    dataframe <- read.csv(paste("KAWN_SensorData_", sitename, ".csv", sep = ""), 
                          header = TRUE)
    dataframe$dateTime <- ymd_hms(dataframe$dateTime, tz = "America/Chicago")
  }
...
steve <- dischargeEstimate(steve, 
                           timeStart = ymd_hms("2018-01-30 17:00:00", tz = "America/Chicago"))
eric <- dischargeEstimate(eric, 
                          timeStart = ymd_hms("2018-02-19 13:00:00", tz = "America/Chicago"))
```
...
eric <- merge(eric, PAR, by = "dateTime") # PAR units are micromoles per m^2 per second
steve <- merge(steve, PAR, by = "dateTime")
desoto <- merge(desoto, PAR, by = "dateTime")
...
BarPress <- data.frame(
  dateTime = ymd_hms(BarPress$startDateTime, tz = "America/Chicago"),
  barom.pressure = BarPress$staPresMean # units: kPa
)

eric <- merge(eric, BarPress, by = "dateTime")
steve <- merge(steve, BarPress, by = "dateTime")
desoto <- merge(desoto, BarPress, by = "dateTime")
