...
matchstr <- names(fit2)[grep("^K600_daily\\[\\d+\\]$", names(fit2))]
...
sum(modelfit.desoto$predictions$ER > 0, na.rm = T) # Count of days when ER is positive = 10
sum(modelfit.desoto$predictions$ER > 0, na.rm = T) / 
  length(modelfit.desoto$predictions$ER) * 100 # Percentage of data removed = 9.7%

# For days when GPP is modeled as positive, remove day from
# dataframe - assume data is innaccurately modeled on this day
sum(modelfit.desoto$predictions$GPP < 0) # Count of days when GPP is positive = 6
sum(modelfit.desoto$predictions$GPP < 0) / 
  length(modelfit.desoto$predictions$GPP) * 100 # Percentage of data removed = 5.5%

# Inspect GPP:ER
modelfit.desoto$predictions$PR <- modelfit.desoto$predictions$GPP / 
  abs(modelfit.desoto$predictions$ER)

# Filter data
modelfit.desoto$predictions <- modelfit.desoto$predictions %>% 
  filter(modelfit.desoto$predictions$ER < 0,
         modelfit.desoto$predictions$GPP > 0,
         ...
         modelfit.desoto$predictions$date < "2018-03-04" |
           modelfit.desoto$predictions$date > "2018-03-07",
         # 03-13 small Q bump (dam release)
         modelfit.desoto$predictions$date != "2018-03-13",
         # 03-20 to 03-22 large dam release
         modelfit.desoto$predictions$date < "2018-03-20" |
           modelfit.desoto$predictions$date > "2018-03-22",
         # 03-27 to 03-29 large dam release
         modelfit.desoto$predictions$date < "2018-03-27" |
           modelfit.desoto$predictions$date > "2018-03-29",
         # 03-31 small Q bump (potential dam release) 
         modelfit.desoto$predictions$date != "2018-03-31",
         # 04-14 to 04-20 large dam release
         modelfit.desoto$predictions$date < "2018-04-14" |
           modelfit.desoto$predictions$date > "2018-04-20",
         # 05-3 to 05-7 large dam release
         modelfit.desoto$predictions$date < "2018-05-03" |
           modelfit.desoto$predictions$date > "2018-05-07",
         ) 
...
ggplot(data = metabmodel_b$modeledO2) +
