...
# bp            barometric pressure [units] --- should be unneeded for us as we already have sat o2
# tt            travel time [units]
# Kmean   
# Ksd
# nbatch
# scale
...
twostationpostsum <- function(O2data, upName, downName, start, z, tt, Kmean, Ksd, 
  ...
  pred.metab <- data.frame(dateList, GPP, GPP.lower, GPP.upper, ER, ER.lower, 
                            ER.upper, K, K.lower, K.upper, s, s.lower, s.upper)
  
  ### Calculate model-predicted O2 values from median metabolism estimates ########
  # Initialize empty vectors to store modeled O2 data
  #modeledO2 <- vector(mode = "numeric", length = length(data))
  
  # Seperate the full dataset into upstream and downstream sections
  #updata <- data[data$`river station` == upName,]
  #downdata <- data[data$`river station` == downName,]
  
  # These should be used in checking the fit of the model (comparing to 
  # upstream and downstream O2)
  # Seperate data into upstream and downstream sections
  #timeup <- updata$dtime[1:as.numeric(length(updata$temp)-lag)]
  #timedown <- downdata$dtime[(1+lag):as.numeric(length(downdata$temp))]
  
  #tempup <- updata$temp[1:as.numeric(length(updata$temp)-lag)]
  #tempdown <- downdata$temp[(1+lag):length(downdata$temp)]
  ...
  # Use 50% quantiles of GPP, ER, K to calculate model predicted O2
  # We want to pull the GPP, ER, K on the date 
  #pred.metab$GPP[downdata$date == pred.metab$date]
  
  #for (j in 1:length(pred.metab)) {
    # Iterate for each day of metabolism prediction
  #  date 
  #}
  
  #pred.metab$GPP[pred.metab$date == ]
  
  #for (i in 1:length(oxyup)){
  #  modeledO2[i] <- (oxyup[i] + ((gppr[2]/z)*(sum(light[i:(i+lag)])/sum(light))) + 
  #                 err[2]*tt/z + 
  #                 (Kcor(tempup[i],Kr[2]))*tt*(osat[i] - oxyup[i] + osat[i])/2) / 
  #    (1+ Kcor(tempup[i],Kr[2])*tt/2) 
  #}
  
  # Create output list to return to user
  output <- list(pred.metab = pred.metab, accept = accept)
...
########### twosbayesplot FUNCTION: Plot data ########
## plotting function. Saves data too.
# ARGUMENTS:
# data          Dataframe name of cleaned raw two station data (ex. "TS_S1S2")
# upName        Character name of upstream station (ex. "S1")
# downName      Character name of downstream station (ex. "S2")

twosbayesplot<-function(GPP, ER, data, Kmean, z, tt, upName, downName) {
  ...
  downdata <- data[data$`river station` == downName,]
	...
	# Plot
	#plot(timeup,oxyup, col="blue", type="l", xlab="Time", ylab="Dissolved oxygen (mg/L)", 
	#     cex.lab=1.5, cex.axis=1.5, lwd=3)
  #points(timedown,oxydown)
  #points(timedown, metab, col = "red")
  #text(timedown[85], quantile(oxydown, 0.8), "downstream", col="black")
  #text(timedown[85], quantile(oxydown, 0.75), "model", col="red")
  #text(timedown[85], quantile(oxydown, 0.70), "upstream", col="blue")

  oxymodel <- data.frame(timeup, timedown, metab, oxydown, oxyup)
  return(oxymodel)
  #write.csv(oxymodel, file ="/Users/bobhall/Rivers/oxymodel.csv")
}

########### Call 2-station metabolism modeling function #######################

# Full time series
# Model
tsout <- twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                           data = TS_S1S2, z = as.numeric(z_S1S2.mean[3]), 
                           tt = 2/86400, 
 ...
 # Plot
twosbayesplot(GPP = tsout$gppr, ER = tsout$err, data = TS_S1S2, 
              z = as.numeric(z_S1S2.mean[3]), 
              tt = as.numeric(tt_S1S2[2]), 
              Kmean = as.numeric(K_S1S2.mean[3]), 
              upName = "S1", downName = "S2")

###see caveats on using MCMC to solve for 3 parameters above.

# Time block a, 2018-02-25 thru 2018-03-05
tsout_a <- twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                           O2data = TS_S1S2_a, z = as.numeric(z_S1S2.mean[3]), 
...
plot(x = tsout_b[[1]]$date, y = tsout_b[[1]]$GPP)
plot(tsout_a[[2]])

tsout_b <-twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                           data = TS_S1S2_b, z = as.numeric(z_S1S2.mean[3]), 
                           tt = 2/86400, 
                           Kmean = as.numeric(K_S1S2.mean[3]), 
                           Ksd = as.numeric(K_S1S2.mean[4]), 
                           upName = "S1", downName = "S2", 
...
# need to figure out how to pull each day's ER estimate out of the model
oxymodel_a <- twosbayesplot(GPP = tsout_a$gppr, ER = tsout_a$err, data = TS_S1S2_a, 
              z = as.numeric(z_S1S2.mean[3]), 
              tt = as.numeric(tt_S1S2[2]), 
              Kmean = as.numeric(K_S1S2.mean[3]), 
              upName = "S1", downName = "S2")
# plot
ggplot(data = oxymodel_a) +
  geom_point(aes(x = as.POSIXct(timeup), y = oxyup, col = "Upstream O2")) +
  geom_point(aes(x = as.POSIXct(timedown), y = oxydown, col = "Downstream O2")) +
  geom_point(aes(x = as.POSIXct(timedown), y = metab, col = "Modeled O2"))
