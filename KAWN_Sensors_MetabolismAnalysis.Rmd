...
# Add date column
o2file <- TS_S1S2_a
#o2file <- o2file %>% group_by(date = date(datetime))
# Subset first day of data
o#2file <- subset(o2file, date == unique(o2file$date)[1])
####
ggplot(data = o2file, aes(x = datetime, y = light)) +
  geom_point()
...
nightreg <- function(o2file, nightStart, nightEnd, bp) {
  ...
  temp <- o2night$temp
  oxy <- o2night$oxy
  # Calculate moving average on oxy data ###### left off here #########
  oxyf1 <- filter(as.numeric(o2night$oxy), rep(1/3, 3), sides=2)

#trim the ends of the oxy data
oxyf2<- oxyf1[c(-1,-length(oxyf1))]

##calculate delO/delt
deltaO2<-((oxyf2[-1]-oxyf2[-length(oxyf2)])/5)*1440
...
out<-list(coeff, K600fromO2(mean(temp), coeff[2]))##gives both the regression coefficients and converts the slope to K600
out
}
