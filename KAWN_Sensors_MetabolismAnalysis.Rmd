...
tt_S1S2[2] <- tt_S1S2/86400
names(tt_S1S2) <- c("tt_sec", "tt_days")
...
twostationpostsum <- function(data, upName, downName, start, z, tt, Kmean, Ksd, 
                              nbatch, scale) {
  # Add date column
  data <- data %>% group_by(date = date(datetime))
  ...
  K <- vector(mode = "numeric", length = length(unique(data$date)))
  K.lower <- vector(mode = "numeric", length = length(unique(data$date)))
  K.upper <- vector(mode = "numeric", length = length(unique(data$date)))
  ...
  for (i in 1:length(unique(data$date))){
    ...
    data <- subset(data, date = unique(data$date)[i])
  ...
  predictions <- data.frame(date, GPP, GPP.lower, GPP.upper, ER, ER.lower, 
                            ER.upper, K, K.lower, K.upper, s, s.lower, s.upper)
  output <- list(predictions = predictions, accept = accept)
...
tsout_a <-twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                           data = TS_S1S2_a, z = as.numeric(z_S1S2.mean[3]), 
                           tt = 2/86400, 
                           Kmean = as.numeric(K_S1S2.mean[3]), 
                           Ksd = as.numeric(K_S1S2.mean[4]), 
                           upName = "S1", downName = "S2", 
