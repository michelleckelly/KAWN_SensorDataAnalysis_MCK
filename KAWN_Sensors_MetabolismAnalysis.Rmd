...
tsout_a_origK <- twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                             O2data = TS_S1S2_a, 
...
tsout_a_origK[["accept"]]

tsout_a_NTK <- twostationpostsum(start = c(2.75, -4.75, 2.2, -2.6), 
                             O2data = TS_S1S2_a, 
                             z = as.numeric(z_S1S2.mean[3]), 
                             tt = as.numeric(tt_S1S2[3]), 
...
### Save estimates
saveRDS(tsout_a_NTK, 
        file = "./Outputs/MetabResults_TwoStation_a_nighttimeK.RData")
saveRDS(tsout_a_origK, 
        file = "./Outputs/MetabResults_TwoStation_a_estimatedK.RData")
saveRDS(tsout_b, 
        file = "./Outputs/MetabResults_TwoStation_b_nighttimeK.RData")

## Merge into one dataframe

# Check modeled oxygen
ggplot(data = tsout_a_origK$oxymodel) +
...
ggplot(data = tsout_a_NTK$oxymodel) +
  geom_line(aes(x = timeup, y = oxyup, color = "S1")) +
  geom_line(aes(x = timedown, y = oxydown, color = "S2")) +
  geom_line(aes(x = timedown, y = modeledO2, color = "Modeled O2"))
# Check modeled GPP and ER
ggplot(data = tsout_a_origK$pred.metab) +
...
ggplot(data = tsout_a_origK$pred.metab) +
  ...
  geom_ribbon(aes(x = date, ymin = K.lower, ymax = K.upper, fill = "K"), 
              alpha = 0.5)
ggplot(data = tsout_a_NTK$pred.metab) +
  geom_line(aes(x = date, y = K, color = "K")) +
  geom_ribbon(aes(x = date, ymin = K.lower, ymax = K.upper, fill = "K"), 
              alpha = 0.5)
