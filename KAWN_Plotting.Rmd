...
ggplot(data = uptake.metab.mass.long, aes(x = ReleaseStatus, 
  ...
  stat_boxplot(geom = "errorbar", width = 0.5, 
               position = position_dodge(width = 0.75)) +
  # Add text for significance
  ggpubr::stat_compare_means(aes(group = ReleaseStatus), label = "p.signif", 
...
ggplot(uptake.metab.mass.long, aes(x = factor(month(Date, label = TRUE, abbr = TRUE)), 
  ...
  # Add text for significance
  ggpubr::stat_compare_means(aes(group = factor(month(Date, label = TRUE, abbr = TRUE)))) +
  # Color scales
  scale_color_manual(values = c("S1" = "#FF0000", "S2" = "#23baaf", 
                                "S3" = "#F2AD00"),
                     limits = c("S1", "S2", "S3")) +
