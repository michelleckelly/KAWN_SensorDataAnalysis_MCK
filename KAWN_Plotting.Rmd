...
uptake.long <- 
  uptake %>%
  gather(site, UaNO3_gNm2day, UaNO3_gNm2day.eric, UaNO3_gNm2day.steve, 
...
supply.long <- 
  uptake %>%
  gather(site, Supply_gNm2day, Supply_gNm2day.eric, Supply_gNm2day.steve,
...
```{r Calculation: Stoich Scaling of Metabolism}
...
metab %>%
  select(date, GPP.eric, ER.eric) %>%
  # Calculate autotrophic N assimilation
  mutate(Ua_gNm2day.eric = GPP.eric*1*0.5/12,
         HR = -(abs(ER.eric) - 0.2*GPP.eric),
         Uh_gNm2day.eric = abs(HR)*1/20,
         U_gNm2day.eric = Ua_gNm2day.eric + Uh_gNm2day.eric)

metab %>%
  select(date, GPP.desoto, ER.desoto) %>%
  mutate()
...
K600.desoto <- 
  ...
  summarise(K600_perday = mean(K600_daily_50pct))

# Interpolate for velocity using v = Q/A
# Approximate river cross sectional A using interpolated depth and aerial w
# Grab aerial width - this is from google maps data, same values used in 
# uptake analysis
w.Eric <- 210.9
w.Steve <- 113.4
w.Desoto <- 138.07

# Grab interpolated water depth [m] from model and measured discharge [m3/s]
v.desoto <- 
  modelfit.desoto[["fit"]]@data %>%
  select(date, solar.time, depth, discharge) %>%
  group_by(date) %>%
  filter(date < ymd("2018-05-01") & date >= ymd("2018-02-01")) %>%
  summarise(depth = mean(depth), discharge = mean(discharge)) %>%
  ungroup() %>%
  summarise(depth_m = mean(depth), Q_m3s = mean(discharge), 
            A_m2 = w.Desoto*depth_m, v_ms = Q_m3s/A_m2,
            v_mday = v_ms*86400)
# Grab 
bind_cols(K600.desoto, v.desoto) %>%
  mutate(L_m = 3*v_mday/K600_perday, L_km = L_m/1000)

# Result: Our metabolism estimates at desoto are integrating the cumulative 
# effects of 20.9km above the sensor

K600.eric <- 
  modelfit.eric[["fit"]]@fit[["daily"]] %>%
  select(date, K600_daily_50pct) %>%
  filter(date < ymd("2018-05-01") & date >= ymd("2018-02-01")) %>%
  summarise(K600_perday = mean(K600_daily_50pct, na.rm = T))
v.eric <- 
  ...
  summarise(depth_m = mean(depth), Q_m3s = mean(discharge), 
            ...
            v_mday = v_ms*86400)
bind_cols(K600.eric, v.eric) %>%
  mutate(L_m = 3*v_mday/K600_perday, L_km = L_m/1000)
# Result: Our metabolism estimates at eric are integrating the cumulative 
# effects of 20.5 km above the sensor (likely affected by dam signal)
...
#a <- 
  ...
  geom_smooth(data = subset(uptake.metab.long, model == "GPP" & 
                              ReleaseStatus == "During" & site == "S3"),
              method = "lm", fullrange = FALSE, aes(color = site), 
