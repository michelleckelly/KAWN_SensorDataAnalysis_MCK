...
str(massbal)
# Make massbal long
massbal.long <- tidyr::gather(massbal[c(1, 19:21)], "site", 
                              "NitrateLoad_kgNday", 2:4)
# Rename factors & add columns
massbal.long$site <-  factor(massbal.long$site)
levels(massbal.long$site)[levels(massbal.long$site) == 
                            "NitrateLoad.eric_kgNday"] <- 
  "S1"
levels(massbal.long$site)[levels(massbal.long$site) == 
                                "NitrateLoad.steve_kgNday"] <- 
  "S2"
levels(massbal.long$site)[levels(massbal.long$site) == 
                            "NitrateLoad.desoto_kgNday"] <- 
  "S3"
...
```{r Fig 4: Uptake vs metabolism.... think about a diff fig}
...
# All together graph
ggplot(uptake.metab.long, aes(x = value, 
                                                     y = UaNO3_gNm2day/1000, 
                                                     color = ReleaseStatus)) +
...
```{r Fig 5: Nitrate load vs uptake}
# Fig 5 nitrate load vs nitrate uptake
ggplot(subset(uptake.metab.mass.long, site == "S1"), aes(x = NitrateLoad_kgNday, 
                                                         y = UaNO3_gNm2day/1000, 
                                                         color = ReleaseStatus)) +
