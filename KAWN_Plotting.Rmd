...
uptake$ReleaseStatus <- factor(uptake$ReleaseStatus, 
                               levels = c("During", "After"))
# Adjust from wide to long format
uptake.long <- tidyr::gather(uptake[c(-2:-4, -6:-8, -10:-12)], key = site, 
                             value = UaNO3_gNm2day,
...
uptake.metab <- dplyr::full_join(metab, uptake, by = "date")
uptake.metab$Date <- NULL

# merge with mass balance data
uptake.metab <- dplyr::full_join(uptake.metab, massbal, by = "date")
uptake.metab <- subset(uptake.metab, uptake.metab$date >= ymd("2018-02-01") &
                         uptake.metab$date < ymd("2018-05-01"))
```
...
ggplot(data = metab.long, aes(ReleaseStatus, value, color= site)) +
  ...
  ggpubr::stat_compare_means(aes(group = ReleaseStatus), label = "p.format") +
  # Axis labels
...
ggsave("./Plots/MetabPlot.png", height = 2.75, width = 6, units = "in")

# N load plot
ggplot(data = massbal, aes(x = date)) +
  # Upstream (Bowersock)
  geom_line(aes(y = NitrateLoad.bowersock_kgNday, color = "S0")) +
  geom_point(aes(y = NitrateLoad.bowersock_kgNday, color = "S0")) +
  ...
  # Discharge (Eric)
  geom_line(aes(y = NitrateLoad.eric_kgNday, color = "S1")) +
  geom_point(aes(y = NitrateLoad.eric_kgNday, color = "S1")) +
  # Downstream (Steve)
  geom_line(aes(y = NitrateLoad.steve_kgNday, color = "S2")) +
  geom_point(aes(y = NitrateLoad.steve_kgNday, color = "S2")) +
  # Desoto
  geom_line(aes(y = NitrateLoad.desoto_kgNday, color = "S3")) +
  geom_point(aes(y = NitrateLoad.desoto_kgNday, color = "S3")) +
...
```{r Uptake vs time significance testing}
...
uptake.long %>%
  ...
  summarise(n = n(),
            mean = mean(UaNO3_gNm2day/1000, na.rm = TRUE),
            sd = sd(UaNO3_gNm2day/1000, na.rm = TRUE))
...
```{r Fig 3: Uptake vs time plotting}
ggplot(data = uptake, aes(x = Date)) +
  # Discharge (Eric)
  geom_smooth(data = subset(uptake, ReleaseStatus == "During"), 
              aes(y = UaNO3_gNm2day.eric/1000, color = "S1"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_smooth(data = subset(uptake, ReleaseStatus == "After"), 
              aes(y = UaNO3_gNm2day.eric/1000, color = "S1"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_point(aes(y = UaNO3_gNm2day.eric/1000, color = "S1")) +
  # Steve
  geom_smooth(data = subset(uptake, ReleaseStatus == "During"), 
              aes(y = UaNO3_gNm2day.steve/1000, color = "S2"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_smooth(data = subset(uptake, ReleaseStatus == "After"), 
              aes(y = UaNO3_gNm2day.steve/1000, color = "S2"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_point(aes(y = UaNO3_gNm2day.steve/1000, color = "S2")) +
  # Desoto
  geom_smooth(data = subset(uptake, ReleaseStatus == "During"), 
              aes(y = UaNO3_gNm2day.desoto/1000, color = "S3"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_smooth(data = subset(uptake, ReleaseStatus == "After"), 
              aes(y = UaNO3_gNm2day.desoto/1000, color = "S3"), 
              formula = y~x, method = "lm", se = FALSE) +
  geom_point(aes(y = UaNO3_gNm2day.desoto/1000, color = "S3")) +
...
# Boxplot
ggplot(data = uptake.long, aes(x = ReleaseStatus, y = UaNO3_gNm2day/1000, 
                               color = site)) +
  facet_wrap(~site) +
  # Error bars
  stat_boxplot(geom = "errorbar", width = 0.5, 
               position = position_dodge(width = 0.75)) +
  # Add text for significance
  stat_compare_means(method = "t.test", comparisons = list(c("During", 
                                                             "After"))) +
  # Boxplots
  geom_boxplot(outlier.shape = NA) +
  # Jitter points
...
# Save
ggsave("./Plots/Uptake_boxplot.png", height = 3.5, width = 4.5, units = "in")
```

```{r Fig 4: Uptake vs metabolism}
# Theres no really strong correlations here. perhaps think about that metabolism may not be driving removal, look at lydia + amys microbe assays to perhaps argue for microbial removal? would that be reflected in ER tho?
uptake.metab <- subset(uptake.metab, date < ymd("2018-05-01"))

ggplot(uptake.metab, aes(x = GPP.eric, y = UaNO3_gNm2day.eric/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)
ggplot(uptake.metab, aes(x = GPP.steve, y = UaNO3_gNm2day.steve/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)
ggplot(uptake.metab, aes(x = GPP.desoto, y = UaNO3_gNm2day.desoto/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)

ggplot(uptake.metab, aes(x = ER.eric, y = UaNO3_gNm2day.eric/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)
ggplot(uptake.metab, aes(x = ER.steve, y = UaNO3_gNm2day.steve/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)
ggplot(uptake.metab, aes(x = ER.desoto, y = UaNO3_gNm2day.desoto/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = FALSE, fullrange = TRUE) +
  facet_grid(.~ReleaseStatus)

str(uptake.metab)
corm <- cor(uptake.metab[c(2,5,11,14,20,23,30:41)], method = "pearson", use = "complete.obs")
```

```{r Fig 5}
# Fig 5 nitrate load vs nitrate uptake

ggplot(uptake.metab, aes(x = NitrateLoad.eric_kgNday, y = UaNO3_gNm2day.eric/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = F, fullrange = TRUE) +
  geom_abline(slope = 1, intercept = 0) +
  facet_grid(.~ReleaseStatus, scale = "free")
ggplot(uptake.metab, aes(x = NitrateLoad.steve_kgNday, y = UaNO3_gNm2day.steve/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = F, fullrange = TRUE) +
  geom_abline(slope = 1, intercept = 0) +
  facet_grid(.~ReleaseStatus, scale = "free")
ggplot(uptake.metab, aes(x = NitrateLoad.desoto_kgNday, y = UaNO3_gNm2day.desoto/1000))+
  geom_point(shape = 21, fill = "white") +
  geom_smooth(formula = y~x, method = "lm", se = F, fullrange = TRUE) +
  geom_abline(slope = 1, intercept = 0) +
  facet_grid(.~ReleaseStatus, scale = "free")
...
ggplot(uptake.metab, aes(x = date))+
  geom_area(aes(y = NitrateLoad.eric_kgNday, fill = "eric load")) +
  geom_area(aes(y= UaNO3_gNm2day.eric/1000, fill = "eric uptake")) +
  geom_line(aes(y= NitrateLoad.eric_kgNday - UaNO3_gNm2day.eric/1000, fill = "difference")) 
  scale_y_log10()
ggplot(uptake.metab, aes(x = date))+
  geom_area(aes(y = NitrateLoad.steve_kgNday, fill = "steve load")) +
  geom_area(aes(y= UaNO3_gNm2day.steve/1000, fill = "steve uptake")) +
  geom_line(aes(y= NitrateLoad.steve_kgNday - UaNO3_gNm2day.steve/1000, fill = "difference"))
  scale_y_log10()
ggplot(uptake.metab, aes(x = date))+
  geom_area(aes(y = NitrateLoad.desoto_kgNday, fill = "desoto load")) +
  geom_area(aes(y= UaNO3_gNm2day.desoto/1000, fill = "desoto uptake")) +
  geom_line(aes(y= NitrateLoad.desoto_kgNday - UaNO3_gNm2day.desoto/1000, fill = "difference"))
  scale_y_log10()

uptake.metab.long <- tidyr::gather(uptake[c(-2:-4, -6:-8, -10:-12)], key = site, 
                             value = UaNO3_gNm2day,
                             UaNO3_gNm2day.eric:UaNO3_gNm2day.desoto)
ggplot(data = uptake.metab, aes(NitrateLoad.eric_kgNday, UaNO3_gNm2day.eric/1000), group = site) +
  geom_boxplot() +
  facet_grid(.~ReleaseStatus, scales = "free_x")
