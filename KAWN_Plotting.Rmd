...
#(metab_desoto)[2:10] <- paste0(names(metab_desoto)[2:10], ".desoto")
...
metab.long <- tidyr::gather(metab[c(-3:-4, -6:-13, -15:-16, -18:-22)], key = site, value = value,
                            GPP.S1S2:ER.S3)
...
#a <- 
  ...
  geom_vline(xintercept = ymd("2018-04-01"), linetype = 2) +
  # Dots
  geom_ribbon(aes(ymin = GPP.lower.S1S2, ymax = GPP.upper.S1S2), fill = "lightgrey") +
...
#b <- 
  ...
  geom_vline(xintercept = ymd("2018-04-01"), linetype = 2) +
  # Dots
  geom_ribbon(aes(ymin = GPP.lower.S3, ymax = GPP.upper.S3), fill = "lightgrey") +
...
# ER, two station S1 S2
#c <- 
  ggplot(data = metab, aes(x = date)) +
  # Add vertical line at release stop date
  geom_vline(xintercept = ymd("2018-04-01"), linetype = 2) +
  # Dots
  geom_ribbon(aes(ymin = ER.lower.S1S2, ymax = ER.upper.S1S2), fill = "lightgrey") +
  geom_point(aes(y = ER.S1S2, fill = "S1"), size = 2, shape = 21) +
  # Axis limits
  scale_x_date(limits = c(as.Date("2018-02-01"), as.Date("2018-05-01"))) +
...
#d <- 
  ...
  geom_vline(xintercept = ymd("2018-04-01"), linetype = 2) +
  # Dots
  geom_ribbon(aes(ymin = ER.lower.S3, ymax = ER.upper.S3), fill = "lightgrey") +
  geom_point(aes(y = ER.S3, fill = "S3"), size = 2, shape = 21) +
  # Axis limits
  scale_x_date(limits = c(as.Date("2018-02-01"), as.Date("2018-05-01"))) +
  #scale_y_continuous(limits = c(-15, 5)) +
  # Axis labels
...
# Average stats
mean(subset(metab_S1S2, date >= "2018-02-01" & date < "2018-05-01")$GPP, 
            na.rm = TRUE)
sd(subset(metab_S1S2, date >= "2018-02-01" & date < "2018-05-01")$GPP, 
            na.rm = TRUE)
mean(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$GPP, 
            na.rm = TRUE)
sd(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$GPP, 
            na.rm = TRUE)
mean(subset(metab_S1S2, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)
sd(subset(metab_S1S2, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)
mean(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)
sd(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)

mean(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$GPP / 
       subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)
sd(subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$GPP / 
       subset(metab_desoto, date >= "2018-02-01" & date < "2018-05-01")$ER, 
            na.rm = TRUE)
...
a <- 
  ...
  #ggpubr::stat_cor(label.y.npc = 0.2, digits = 2,
...
b <- 
  ...
  geom_smooth(data = subset(vf, site == "S2"), method = lm, 
              color = "black", fill = "grey") +
  # Add points
  geom_point(shape = 21) +
  # Add R2 and p value labels to plot
  #ggpubr::stat_cor(label.y.npc = 0.9, digits = 2,
  #                 aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~"))) +
  # Labels
  labs(tag = "b") +
  xlab(NULL) +
...
## Correlation testing
# Full time series
cor.test(~UaNO3_gNm2day.eric+Ustoic_gNm2day.S1S2, 
         data = stoich)
cor.test(~UaNO3_gNm2day.steve+Ustoic_gNm2day.S1S2, 
         data = stoich)
...
# Eric
a <- 
  ggplot(data = stoich, aes(x = Ustoic_gNm2day.S1S2, 
                          y = UaNO3_gNm2day.eric, 
                          shape = ReleaseStatus)) +
  # 1:1 line
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  # Data points
  geom_point(fill = "#FF0000", size = 2) +
...
a <- 
  ...
  theme(legend.position = "none", legend.title = element_blank(),
        ...
        axis.text = element_text(color = "black"),
        text = element_text(size = 8)) + 
  expand_limits(x = 17, y = 3) # expand axis limits so you have room to put stats in
# GPP vs Vf
b <- 
  ...
  theme(legend.position = "none", legend.title = element_blank(),
        ...
        axis.text = element_text(color = "black"),
        text = element_text(size = 8)) + 
  expand_limits(y = 6.5, x = 19) # expand axis limits so you have room to put stats in

grob <- grid.arrange(a,b, ncol = 2)
ggsave("./Plots/GPP_U_Vf.png", grob, height = 127, width = 115, 
       units = "mm", dpi = 600)
...
b <- 
  ...
  theme(legend.position = "none", legend.title = element_blank(),
        ...
        axis.text = element_text(color = "black"),
        text = element_text(size = 8))+ 
  expand_limits(y = 4.5) # expand axis limits so you have room to put stats in

grob <- grid.arrange(a,b, ncol = 2)
ggsave("./Plots/ER_U_Vf.png", grob, height = 127, width = 115, 
       units = "mm", dpi = 600)
```
