...
uptake.metab.mass.long$Month <- lubridate::month(uptake.metab.mass.long$Date)
ggpubr::compare_means(UaNO3_gNm2day~site, data = uptake.metab.mass.long, 
                      method = "kruskal.test")
# Test uptake during vs after
ggpubr::compare_means(UaNO3_gNm2day~ReleaseStatus, data = uptake.metab.mass.long, 
...
# S1
cor.test(~UaNO3_gNm2day+value, 
         data = subset(uptake.metab.mass.long, model == "GPP" & site == "S1"),
         method = c("spearman"))
# S2
cor.test(~UaNO3_gNm2day+value, 
         data = subset(uptake.metab.mass.long, model == "GPP" & site == "S2"),
         method = c("spearman"))
# S3
cor.test(~value+UaNO3_gNm2day, 
         data = subset(uptake.metab.mass.long, model == "GPP" & site == "S3"),
         method = c("spearman"))
ggpubr::stat_cor()
```
...
mean(massbal$Ammonium.bowersock_mgNL[massbal$date >= "2018-02-01" &
                                      massbal$date <= "2018-05-01"], na.rm = TRUE)
...
a <- 
  ...
  geom_smooth(data = subset(uptake.metab.mass.long, model == "GPP" & ReleaseStatus == "During"),
              method = "lm", fullrange = TRUE, aes(color = ReleaseStatus), 
              fill = "lightgrey", alpha = 0.2) +
  # Linear model for after release
  geom_smooth(data = subset(uptake.metab.mass.long, model == "GPP" & ReleaseStatus == "After"),
  ...
  ggpubr::stat_cor(data = subset(uptake.metab.mass.long, model == "GPP"),
                   method = "spearman", label.x = c(7), 
                   label.y = c(3700, 3200, 500, 420, 950, 800), size = 3,
...
b <- 
  ...
  facet_grid(site~., scales = "free", switch = "y") +
  # Linear model for During release
  geom_smooth(data = subset(uptake.metab.mass.long, model == "ER" & ReleaseStatus == "During"),
              method = "lm", fullrange = TRUE, aes( color = ReleaseStatus), 
              fill = "lightgrey", alpha = 0.2) +
  # Linear model for after release
  geom_smooth(data = subset(uptake.metab.mass.long, model == "ER" & ReleaseStatus == "After"),
              method = "lm", fullrange = TRUE, aes( color = site), 
              fill = "lightgrey", alpha = 0.6) +
  # Plot points on top
  geom_point(size = 2) +
...
ggplot() +
  # Meta-analysis data
  geom_point(data = tank, aes(x = Discharge_Ls/1000, y = NO3_Vf_mmmin*NO3_Cb_mgNL/1000*1440), 
...
ggplot() +
  ...
  # Our data
  geom_point(data = means, aes(x = Discharge_m3s.Overall, 
                               y = U_gNm2day.Overall/Nitrate_mgL.Overall*1000/1440, 
                               fill = Site), 
             size = 3, shape = 21) +
  # Axis scales
  scale_x_log10() +
