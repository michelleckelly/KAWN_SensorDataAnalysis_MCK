...
desoto.w.data.apr <- desoto.w.data[
  ...
  (desoto.w.data$date < ymd_hms("2018-04-08 00:00:00")),]

plot(desoto.w.data.apr$DO)
plot(desoto.w.data.apr$DO.sat)

str(desoto.w.data.apr)
...
analyze.wavelet(desoto.w.data.apr, my.series = "DO.sat", 
                dt = 1/(4*24), lowerPeriod = 0.125, upperPeriod = 3,
                make.pval = TRUE, date.format = "%Y-%m-%d %H:%M:%S")
wt.image(desoto.w, periodlab = "Period (days)", timelab = "Date", 
         main = "Wavelet: DO % saturation",
         label.time.axis = TRUE, show.date = TRUE, date.format = "%Y-%m-%d")




#cross wavelet power of 2 time series, first make combo dataframe and place into desoto.w.data.apr spot
analyze.coherency(desoto.w.data.apr, my.pair = c(3,4), dt = 4, 
                  date.format = "%Y-%m-%d %H:%M:%S")

data.frame(desoto$nitrateNitrite)
```