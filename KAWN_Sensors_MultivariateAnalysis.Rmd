...
# Make sure datetime is the correct class
...
# Convert UTC to local time (be sure everything agrees with everything else...)
eric_data$DateTime_CST <- lubridate::with_tz(eric_data$DateTime_UTC, 
                                              tzone = "America/Chicago")
steve_data$DateTime_CST <- lubridate::with_tz(steve_data$DateTime_UTC, 
                                              tzone = "America/Chicago")
desoto_data$DateTime_CST <- lubridate::with_tz(desoto_data$DateTime_UTC, 
                                              tzone = "America/Chicago")
...
```{r NO3Uptake.Desoto}
# Calculate NO3 uptake at the Desoto sensor, 
# using the extrapolated diel method 
# (Heffernan and Cohen 2010, Eqn. 1)

# Remove any data that's been flagged "Bad"
desoto_data$value[desoto_data$flagtype %in% c("Bad Data")] <- NA
# Keeping the flagtype and flagcomment columns screws up the
# reshape, so remove these
desoto_data$flagtype <- NULL
desoto_data$flagcomment <- NULL
# Reshape the data from long format to wide format
desoto_data <- tidyr::spread(desoto_data, key = variable, value = value)

# Create new dataframe with data needed for uptake calculation
desoto_uptake <- desoto_data[c("DateTime_UTC", "DateTime_CST", 
                               "Discharge_m3s", "Nitrate_mgL")]

# Add a vector that indexes by date
desoto_uptake$date <- lubridate::date(desoto_uptake$DateTime_CST)
for (i in 1:nrow(desoto_uptake)){
  desoto_uptake$day[i] <- as.numeric(desoto_uptake$date[i] - desoto_uptake$date[1])
}

# Create dataframe MaxNitrate: maximum [NO3-] each day
MaxNitrate <- 
  desoto_uptake %>%
  dplyr::group_by(date = lubridate::floor_date(DateTime_CST, unit = "day")) %>%
  dplyr::summarise(MaxNitrate = max(Nitrate_mgL, na.rm = TRUE))
MaxNitrate$date <- as.Date(MaxNitrate$date)

# Merge dataframes, joining matching rows by "date"
desoto_uptake <- left_join(desoto_uptake, MaxNitrate)
# Remove "date"
desoto_uptake$date <- NULL

# Use a static value of river bed area (m^2)
# (until we have modeling data)
...
# average discharge:
mean(desoto_uptake$Discharge_m3s, na.rm = TRUE) # 61.3 m3/s
# therefore, will approximate channel area as 130 m2
A_m2 <- 130

# Convert discharge to m3/hr so units work out
desoto_uptake$Discharge_m3hr <- desoto_uptake$Discharge_m3s*3600
# Calculate water velocity
desoto_uptake$v_mhr <- desoto_uptake$Discharge_m3hr/A_m2

# Calculate the difference between the preceeding 
# NO3-max and [NO3-] at time t
desoto_uptake$NO3Diff <- NA
# Take the first row that day == 1
start <- which(desoto_uptake$day == 1)[1]
for (i in start:nrow(desoto_uptake)){
  # Calculate previous day
  prevday <- desoto_uptake$day[i] - 1
  # Get maximum concentration of NO3 during previous day
  NO3Max.prevday <- unique(desoto_uptake$MaxNitrate[desoto_uptake$day == prevday])
  # Get current NO3 concentration
  NO3 <- desoto_uptake$Nitrate_mgL[i]
  # Take the difference between previous max and current
  desoto_uptake$NO3Diff[i] <- NO3Max.prevday - NO3
  # Iterate loop and end
  i <-  i + 1
}
...
desoto_uptake <- 
  desoto_uptake %>%
  group_by(day) %>%
  mutate(NO3Sum = sum(NO3Diff))

# Multiply sum difference and velocity * 0.001 L/m3 * 24hr/1day
desoto_uptake$UaNO3 <- desoto_uptake$NO3Sum * desoto_uptake$v_mhr * 0.001 * 24

ggplot(data = desoto_uptake, aes(DateTime_CST, UaNO3)) +
  geom_line() +
  geom_point()
ggplot(data = modelfit.desoto, aes(date, GPP)) +
  geom_point()
