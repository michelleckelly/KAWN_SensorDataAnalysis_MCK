...
dailyavg <- function(sensor_data, metabolism_model){
  ...
  sensor_data <- sensor_data %>%
    ...
    dplyr::summarise(Q_m3s.mean = mean(na.omit(Discharge_m3s)),
...
fit <- lm(eric.dailyavg$GPP ~ eric.dailyavg$"Q_m3s.mean")
signif(summary(fit)$coef[2,4], 3)
eric.dailyavg[[xvariable]]
xvariable

# linear fit
fita <- lm(eric.dailyavg$GPP ~ eric.dailyavg$xvariable)
fit <- summary(lm(eric.dailyavg[["GPP"]] ~ eric.dailyavg[[xvariable]]))
fit$coefficients[2,4]
min(na.omit(eric.dailyavg$GPP))
max(na.omit(eric.dailyavg[[xvariable]]))*2/3
