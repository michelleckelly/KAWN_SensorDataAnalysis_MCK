...
Qdata$Flow_Inst <- ft3s_m3s(Qdata$Flow_Inst)
Qdata <- Qdata[c(-1,-5,-6)]

# take average flow for each day
Qdata.avg <- 
  Qdata %>%
    group_by(site_no, day = floor_date(dateTime, "day")) %>%
    summarize(mean.Q = mean(Flow_Inst))

Qdata.avg <- spread(Qdata.avg, site_no, mean.Q)
Qdata.avg$day <- ymd(Qdata.avg$day)
names(Qdata.avg)[names(Qdata.avg)=="06891080"] <- "Q1.lawrence" 
names(Qdata.avg)[names(Qdata.avg)=="06891500"] <- "Q3.wakarusa" 
names(Qdata.avg)[names(Qdata.avg)=="06892000"] <- "Q4.stranger" 
names(Qdata.avg)[names(Qdata.avg)=="06892350"] <- "Q5.desoto" 

# load in farmland Q
FarmlandReport <- read.csv("FarmlandDailyReport_MCKformatted.csv", 
...
Qdata.avg <- merge(Qdata.avg, Q2.Farmland, by.x = "day", by.y = "Date", all = TRUE)
names(Qdata.avg)[names(Qdata.avg)=="flowRate_Outfall001A_gpm"] <- "Q2.Farmland"

# pumping stops on apr 1
Qdata.avg$Q2.Farmland[Qdata.avg$day > ymd("2018-04-01")] <- 0
...
Qdata.avg$upstreamSum <- Qdata.avg$Q1.lawrence + Qdata.avg$Q2.Farmland + Qdata.avg$Q3.wakarusa + Qdata.avg$Q4.stranger

Qdata.avg$waterBal_percentDev <- ((Qdata.avg$Q5.desoto - Qdata.avg$upstreamSum) /
                                    Qdata.avg$upstreamSum)*100
plot(x = Qdata.avg$day, y = Qdata.avg$waterBal_percentDev)
```
percent deviation = ((downstream Q - upstream Q) / upstream Q) * 100%

negative values: upstream (sum Lawrence gauge, Farmland flow, Wakarusa R., Stranger Creek) has greater Q  
