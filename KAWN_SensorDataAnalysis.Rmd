...
#head(merge(bowersock_N$`Date and time`, bowersockN_daynightdiff, all.x = TRUE))
head(lag(bowersock_N$`Date and time`, k = 2))

ggplot() +
  geom_freqpoly(data = bowersockN_daynightdiff)

plot.ts()
plot.ts(diff(steve_N$`Nitrate plus nitrite as mg-N/L`, lag = 49, differences = 1,
     object = steve_N$`Date and time`))
plot.ts(diff(desoto$nitrateNitrite, lag = 49, differences = 1,
     object = desoto$dateTime))
# find a way to step between solar noon and midnight

# taking the first derivative of eric's N data
eric_firstderiv <- data.frame(
  eric_N$`Date and time`[2:length(eric_N$`Date and time`)],
  diff(eric_N$`Nitrate plus nitrite as mg-N/L`, lag = 1, differences = 1, 
       object = eric_N$`Date and time`)*100
)
names(eric_firstderiv) <- c("Date and time", "Rate of change")

ggplot() +
  geom_point(data = eric_firstderiv, aes(x = eric_firstderiv$`Date and time`,
                                         y = eric_firstderiv$`Rate of change`,
                                         color = "site 2"), show.legend = FALSE) +
  theme_classic() +
  coord_cartesian(xlim = c(pullBegin, pullEnd)) +
...
# steve's
steve_firstderiv <- data.frame(
  steve_N$`Date and time`[2:length(steve_N$`Date and time`)],
  diff(steve_N$`Nitrate plus nitrite as mg-N/L`, lag = 1, differences = 1, 
       object = steve_N$`Date and time`)*100
)
names(steve_firstderiv) <- c("Date and time", "Rate of change")

ggplot() +
  geom_point(data = steve_firstderiv, aes(x = steve_firstderiv$`Date and time`,
                                         y = steve_firstderiv$`Rate of change`,
                                         color = "site 3"), show.legend = FALSE) +
  theme_classic() +
  coord_cartesian(xlim = c(pullBegin, pullEnd)) +
  labs(y = "Rate of change (%)", x = "Date") +
  theme(axis.text = element_text(size = 14),
...
```{r MetabolismSetup, fig.height=7, fig.width=10}
...
desoto_MetabData_GH <- subset(desoto_MetabData, select = -c(depth))
...
mm_mle_desoto <- metab(mle_specs, data = desoto_MetabData_GH)
...
mm_mle_desoto_plot <- 
  ...
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 14))
mm_mle_desoto_plotDO <- 
  plot_DO_preds(mm_mle_desoto) +
  theme_classic() +
  theme(axis.text = element_text(size = 14),
        axis.title = element_text(size = 14))

ggsave(filename = "Metabolism_desoto_mle.png", plot = mm_mle_desoto_plot,
       dpi = 500, width = 10, height = 5, units = "in")
ggsave(filename = "Metabolism_desoto_mle_DO.png", plot = mm_mle_desoto_plotDO,
       dpi = 500, width = 10, height = 5, units = "in")

  # trying a bayseian model for better prediction of multi-day metabolism
bayes_name <- mm_name(type = "bayes", pool_K600 = "linear")
bayes_specs <- specs(bayes_name)
mm_bayes_desoto <- metab_bayes(specs = bayes_specs, data = desoto_MetabData_GH)

# fitting a bayesian model (better for multiple days of data b/c it can pool k600 values)
model_name <- mm_name(type = "bayes", pool_K600 = "binned",
