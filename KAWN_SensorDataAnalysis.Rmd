---
title: "KAWN RAPID: Working .Rmd File for Sensor Data Analysis"
...
1. .xml to .csv conversion of raw Nitratax files  
2. data cleanup of Nitratax files  
3. data loading from miniDOT files  
4. data pull from USGS server  
5. basic plotting of nitrate+nitrite, dissolved oxygen, and temperature data for each site  

Editing notes for future:
1. setup script so nitratax .csvs, neon data is saved in a nested folder

Setting up .Rmd environment and loading packages for script  
...
.xml to .csv conversion function  
```{r Nitratax_XMLtoCSV, include=TRUE}
# Function name:
      # Nitratax_XMLtoCSV
...
Import of Nitratax data  
```{r NitrataxLoading, include=TRUE}
...
if (loadfromCSV == TRUE){
  bowersock_N <- read.csv(file = "KansasR_Nitratax_Bowersock.csv", header = TRUE,
                         colClasses = c("POSIXct", "numeric"))
  eric_N <- read.csv(file = "KansasR_Nitratax_Eric.csv", header = TRUE,
                         colClasses = c("POSIXct", "numeric"))
  steve_N <- read.csv(file = "KansasR_Nitratax_Steve.csv", header = TRUE,
                         colClasses = c("POSIXct", "numeric"))
...
```{r NitrataxCleanup, include=TRUE}
if (loadfromCSV == FALSE){
...
```{r DOloading, include=TRUE}
steve_DO <- read.table(file = "miniDOT_Steve_Pulled03-30-2018.txt", header = TRUE,
...
```{r DesotoPull, include=TRUE, warning=FALSE}
...
pullEnd <- as.POSIXct("2018-04-08", format = "%Y-%m-%d") # Date for end of data pull
...
Data cleanup from USGS server: rename columns, convert to SI units
...
# convert from american to SI units
lawrence$Flow_Inst <- ft3s_Ls(lawrence$Flow_Inst)
lawrence$.Downstream.of.Bowersock.Dam._GH_Inst <-
  ft_m(lawrence$.Downstream.of.Bowersock.Dam._GH_Inst)
lawrence$.Upstream.of.Bowersock.Dam._GH_Inst <-
  ft_m(lawrence$.Upstream.of.Bowersock.Dam._GH_Inst)
```

Water balance: downstream - upstream discharge
```{r waterBalance, include=TRUE}
#plotting difference of flow between upstream and downstream
ggplot() +
...
#calculate difference between downstream and upstream gauge
...
waterBal_plot <- ggplot() +
  ...
  geom_hline(aes(yintercept = mean(na.omit(waterBal$difference))),
    ...
    coord_cartesian(xlim = c(pullBegin, pullEnd), 
                    ylim = c(-30000, 50000)) +
    labs(y = "Difference between downstream flow gauge and upstream flow gauge, L/s",
         x = "Date", 
...
```{r FarmlandLogsLoading, include=TRUE}
FarmlandPumpingLogs <- read.csv(file = "FarmlandDailyReport_MCKformatted.csv", 
                                header = TRUE, sep = ",")
```

Data cleanup from City Report: conversion to metric
...
FarmlandPumpingLogs$VolumePumped_MG <- 
  MG_L(FarmlandPumpingLogs$VolumePumped_MG)
names(FarmlandPumpingLogs)[names(FarmlandPumpingLogs) == "VolumePumped_MG"] <-
  "VolumePumped_L"
...
FarmlandPumpingLogs$flowRate_Outfall001A_gpm <-
  gpm_Ls(FarmlandPumpingLogs$flowRate_Outfall001A_gpm)
names(FarmlandPumpingLogs)[names(FarmlandPumpingLogs) == "flowRate_Outfall001A_gpm"] <- 
  "flowRate_Outfall001A_L.s"

FarmlandPumpingLogs$flowRate_AveragePump_gpm <- 
  gpm_Ls(FarmlandPumpingLogs$flowRate_AveragePump_gpm)
names(FarmlandPumpingLogs)[names(FarmlandPumpingLogs) == "flowRate_AveragePump_gpm"] <- 
  "flowRate_AveragePump_L.s"
...
IN DEVELOPMENT (NEON server pull)
```{r NEONpull, include=TRUE, eval=FALSE}
# Pull PAR data for March
getPackage(dpID = "DP1.00024.001", site_code = "UKFS",
           year_month = "2018-03", package = "basic",
           savepath = getwd())
# Join months of PAR data into one .csv file
stackByTable(dpID = "DP1.00024.001", filepath = "~/KAW_N/R/NEON.D06.UKFS.DP1.00024.001.2018-03.basic.20180409T182009Z.zip", savepath = getwd(), 
             package = "basic")
#UKFS_PAR_March18 <- read.table(unz(description = ))
get
```

N data from all core sites on one plot  
```{r NmergedPlot, include=TRUE, warning=FALSE}
...
```{r simplePlotFunction, include=TRUE}
# Function name:
      # simplePlot
# Function description:
...
simplePlot <- function(dataset, xAxis, yAxis, plotBegin, plotEnd, 
                       yMin, yMax, xLabel, yLabel, title){
  ggplot(NULL) + 
...
Plot Bowersock data (Upstream site)
```{r bowersockPlots, include=TRUE, warning=FALSE}
...
print(bowersockPlot_N)
ggsave("bowersockPlot_N.png", width = 10, height = 3, dpi = 120)
```


Plot data fromFarmland pumping logs (at discharge pipe)
```{r FarmlandPumpLogsPlots, include=TRUE, warning=FALSE}
...
FarmlandPump_DrainageDitch_NitratePlot <- ggplot(data = FarmlandPumpingLogs, 
    ...
    labs(y = "Nitrate plus nitrite [mg-N/L]", x = "Date", 
         title = "Farmland discharge ditch") +
...
Plot Eric data
```{r ericPlots, include=TRUE, warning=FALSE}
...
ggsave("ericPlot_N_DO_Temp.png", 
       plot = grid.arrange(ericPlot_N, ericPlot_DO, ericPlot_temp, ncol = 1, nrow = 3),
       width = 10, height = 8, dpi = 120)
```

Plot Steve data (5 km Downstream)
```{r stevePlots, include=TRUE, warning=FALSE}
# Steve (downstream)
...
ggsave("stevePlot_N_DO_Temp.png", 
       plot = grid.arrange(stevePlot_N, stevePlot_DO, stevePlot_temp, ncol = 1, nrow = 3),
       width = 10, height = 8, dpi = 120)
```

Plot Desoto data (30 km Downstream)
```{r desotoPlots, include=TRUE, warning=FALSE}
...
ggsave("desotoPlot_N_DO_Temp.png", 
       plot = grid.arrange(desotoPlot_N, desotoPlot_DO, desotoPlot_temp, 
                           ncol = 1, nrow = 3),
       width = 10, height = 8, dpi = 120)
```
