...
```{r NEONpull_cleanup, include=FALSE, eval=TRUE}
...
desoto$depth <- calc_depth(desoto$Flow_Inst)
  # this gives us ~10m depth!! WRONG! is gage height more representative of actual depth?? ask USGS
...
head(intersect(desoto$dateTime_solar, PAR$dateTime_solar))

head(merge.data.frame(desoto, PAR, 
                      by = intersect(desoto$dateTime_solar, PAR$dateTime_solar),
                      incomparables = NULL))
