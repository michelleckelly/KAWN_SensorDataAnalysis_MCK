...
b <- 
  ...
  geom_point(shape = 21) +
  # Add regression line to S3
  geom_smooth(data = subset(vf, site == "S3"), method = lm, 
              color = "black", fill = "grey") +
  # Labels
  labs(tag = "b") +
  xlab(NULL) +
...
a <- 
  ...
  coord_cartesian(xlim = c(0, 0.25), ylim = c(-0.1, 3)) +
...
a <- 
  ...
  # Axis limits
  coord_cartesian(xlim = c(-0.5, 12), ylim = c(-1, 3)) +
...
c <- ggplot(data = NULL, aes(x = ER.S1S2, y = UaNO3_gNm2day.steve)) +
  ...
  geom_point(data = subset(stoich20, ReleaseStatus == "After"),
             fill = "#23baaf", size = 2, shape = 21) +
  # Axis limits
  coord_cartesian(xlim = c(-0.5, 6), ylim = c(-1, 3)) +
...
meanData <- data.frame(GPP.mean = c(NA,
           ...
           Q.mean = c(mean(uptake$Mean.Discharge_m3s.eric, na.rm = TRUE),
                      mean(uptake$Mean.Discharge_m3s.steve, na.rm = TRUE),
                      mean(uptake$Mean.Discharge_m3s.desoto, na.rm = TRUE)), 
...
a <- 
  ...
  # Axis scales
  scale_x_log10(labels = scales::number_format(accuracy=0.01, scale=1, 
                                               big.mark = "")) +
  scale_y_log10(labels = scales::number_format(accuracy=0.01, scale=1,
                                               big.mark="")) +
  # Axis labels
