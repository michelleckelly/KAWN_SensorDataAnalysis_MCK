...
stoich.S1S2 <- stoich.S1S2 %>%
  select(Date = date, GPP.S1S2 = GPP, ER.S1S2 = ER, Ustoic_gNm2day.S1S2 = U_gNm2day)
stoich.desoto <- stoich.desoto %>%
  select(Date = date, GPP.S3 = GPP, ER.S3 = ER, Ustoic_gNm2day.S3 = U_gNm2day)
...
mean(metab[na.omit(metab$GPP.S1S2),]$GPP.S3) # Mean of GPP at S3 during S1S2 data
...
mean(stoich$Ustoic_gNm2day.desoto, na.rm = T) # S3 U pred
mean(stoich[-which(is.na(stoich$Ustoic_gNm2day.desoto)),]$UaNO3_gNm2day.desoto,
...
lmmodel <- lm(stoich$Ustoic_gNm2day.desoto ~ stoich$UaNO3_gNm2day.desoto) # S3
summary(lmmodel) # S3
# During waste release
lmmodel <- lm(Ustoic_gNm2day.desoto ~ UaNO3_gNm2day.desoto,
...
a <- 
  ...
  ggpubr::stat_cor(label.y.npc = 0.2, digits = 2, size = 2, method = "pearson",
...
b <- 
  ...
  # Axis limits
  scale_x_continuous(limits = c(-0.5, 15.5), breaks = c(0, 5, 10, 15)) +
  scale_y_continuous(limits = c(-0.5, 4.5), breaks = c(0, 2, 4)) +
  # Axis labels
  xlab(bquote("GPP (g" ~ O[2] ~ m^-2 ~ d^-1 ~ ")")) + 
  ylab(expression(U[Diel] ~ "(g N" ~~ m^{-2} ~ d^{-1} * ")")) +
  labs(tag = "b") +
...
# Plot
a <- ggplot(data = NULL, aes(x = ER.S1S2, y = UaNO3_gNm2day.steve)) +
  # Add regression line
  geom_smooth(data = stoich, method = lm, color = "black", 
              fill = "grey") +
  # Data points
...
b <- 
  ...
  geom_smooth(data = tank, aes(x = Discharge_Ls/1000, 
                              y = NO3_Vf_mmmin*(NO3_Cb_mgNL/1000)*1440), 
              method = "lm", fullrange = TRUE, color = "black") +
