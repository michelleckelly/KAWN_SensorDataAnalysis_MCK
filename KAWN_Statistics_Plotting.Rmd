...
```{r pull together flow and width data for admin}
channel_S1 <- diel_eric %>% select(DateTime_UTC, Discharge_m3s)
channel_S1 <- na.omit(channel_S1)
channel_S1$Width_m <- 210.9
channel_S1$EstimatedDepth_m <- 0.409*channel_S1$Discharge_m3s^0.294 # As in MS

channel_S2 <- diel_steve %>% select(DateTime_UTC, Discharge_m3s)
channel_S2 <- na.omit(channel_S2)
channel_S2$Width_m <- 113.4
channel_S2$EstimatedDepth_m <- 0.409*channel_S2$Discharge_m3s^0.294

channel_S3 <- diel_desoto %>% select(DateTime_UTC, Discharge_m3s)
channel_S3 <- na.omit(channel_S3)
channel_S3$Width_m <- 138.07
channel_S3$EstimatedDepth_m <- 0.409*channel_S3$Discharge_m3s^0.294

write.csv(channel_S1, file = "SiteS1_QWidth_4Aug21.csv", row.names = FALSE)
write.csv(channel_S2, file = "SiteS2_QWidth_4Aug21.csv", row.names = FALSE)
write.csv(channel_S3, file = "SiteS3_QWidth_4Aug21.csv", row.names = FALSE)
```
...
stoich20 %>% 
  ...
  summarise(mean(UaNO3_gNm2day.desoto, na.rm = T),
            mean(Upred_gNm2day.S3, na.rm = T))
# Revisions: compare C:N = 20 with C:N = 16
(mean(stoich16$Upred_gNm2day.S1S2, na.rm = T) - 
    mean(stoich20$Upred_gNm2day.S1S2, na.rm = T)) / 
  mean(stoich20$Upred_gNm2day.S1S2, na.rm = T) # Percent change
(mean(stoich16$Upred_gNm2day.S3, na.rm = T) - 
  mean(stoich20$Upred_gNm2day.S3, na.rm = T)) / 
  mean(stoich20$Upred_gNm2day.S3, na.rm = T)
...
# S2
a <- 
  ggplot(data = stoich20, aes(x = Upred_gNm2day.S1S2, y = UaNO3_gNm2day.steve)) +
  # 1:1 line
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  # Data points
  geom_point(data = subset(stoich20, ReleaseStatus == "During"), fill = "white", 
...
grob <- grid.arrange(a, b, ncol = 2, nrow = 1)
ggsave(grob, file = "./Plots/Fig5_U_Upred.png", height = 95, width = 115*2-50, 
       units = "mm", dpi = 600)
```
...
a <- 
  ...
  # Data points
  geom_point(data = subset(stoich20, ReleaseStatus == "During"), 
             fill = "white", size = 2, shape = 21) +
  geom_point(data = subset(stoich20, ReleaseStatus == "After"),
...
b <- 
  ...
  # Add regression line
  geom_smooth(data = stoich20, method = lm, color = "black", 
              fill = "grey") +
  # Data points
  geom_point(data = subset(stoich20, ReleaseStatus == "During"), 
             fill = "white", size = 2, shape = 21) +
...
a <- 
  ...
  scale_y_log10(labels = scales::number_format(accuracy=0.1, scale=1)) +
  # Axis labels
...
d <- 
  ...
  geom_point(data = meanData, 
             aes(x = Q.mean, y = Vf.mean, 
                 fill = c("S1", "S2", "S3")), size = 3, shape = 21, alpha = 0.8) +
  # Axis scales
  scale_x_log10(labels = scales::number_format(accuracy=1, scale=1), 
                limits = c(NA, 105)) +
  scale_y_log10(labels = scales::number_format(accuracy=1, scale=1, 
